...
const filterRecipesItens = (dbItens, fieldName) => {
  const dbFilter = Object
    .entries(dbItens[0])
    .filter((key) => key[0].includes(fieldName) && key[1])
    .map((e) => e[1]);

  return dbFilter;
};

export { fetchAPI, fetchByCategories, filterRecipesItens };
